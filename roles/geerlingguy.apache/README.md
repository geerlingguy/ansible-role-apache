...
(Debian/Ubuntu ONLY) Which Apache configs to enable or disable (these will be symlinked into the appropriate location). See the `confs-available` directory inside the apache configuration directory (`/etc/apache2/confs-available` by default) for all the available configs.
